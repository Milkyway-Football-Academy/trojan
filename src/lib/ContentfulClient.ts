...
export class ContentfulClient {
    ...
    async getEntries<T>(entryId: string): Promise<T[]> {
        const entries = await this.client.getEntries<T>({content_type: entryId});
        return entries.items.length ? entries.items.map((ent) => ent.fields) as T[] : []
    }

    async getAssets<T>(assetId: string): Promise<T> {
        const asset = await this.client.getAsset<T>(assetId);
